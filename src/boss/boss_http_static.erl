...
rest_init(Req, Opts) ->
    %error_logger:info_msg("rest_init:Req:~p~n", [Req]),
	...
	State = case check_path(Filepath) of
		...
		ok ->
			{AcceptEncoding, _Req1} = cowboy_http_req:header('Accept-Encoding', Req, []),
			ListAcceptEncoding = string:tokens(binary_to_list(AcceptEncoding), ","),

			{UserAgent, _Req2} = cowboy_http_req:header('User-Agent', Req),
			Msie = case re:run(binary_to_list(UserAgent), "(MSIE 6.0|MSIE 5.5)") of
				   nomatch -> "/";
				   _ -> "_msie"
			       end,
			DocRoot = binary_to_list(Directory1),
			FunCheck = fun(S) ->
					   PathToCheck = DocRoot ++ "_" ++ S ++ Msie,
					   FileToCheck = join_paths([list_to_binary(PathToCheck)],Filepath),
					   case file:read_file_info(FileToCheck) of
					       {ok, _FileInfop} -> true;
					       _ -> false
					   end
				   end,
			ListEncoding = [{X, DocRoot ++ "_" ++ X }||X<-ListAcceptEncoding, FunCheck(X)],
			{Encoding, Filepath1} = case ListEncoding of 
				  [] ->
				  ...
				  Other ->
					  ...
					  {_,_Path} ->
					      File1 = join_paths(_Path, Filepath),
					      error_logger:info_msg("gzip found ~p",[File1]),
					      {gzip, File1};
					  false ->
					      case lists:keyfind("deflate", 1, Other) of %% deflate second
						  {_,_Path1} ->
						      File2 = join_paths(_Path1, Filepath),						     
 						      error_logger:info_msg("deflate found ~p",[File2]),
			...
			#state{filepath=Filepath1, fileinfo=Fileinfo, mimetypes=Mimetypes1, encoding=Encoding,
			       etag_fun=ETagFunction}
		end,
