...
compile_file(ViewPath, Module, Options) ->
    ...
    Res                 = compile(ViewPath, Module, HelperDirModule, TranslatorPid,
                                    OutDir, CompilerOptions, Locales, DocRoot, 
                                    TagHelpers, FilterHelpers, ExtraTagHelpers,
                                    ExtraFilterHelpers),
    case Res of
        {ok, Module} -> {ok, Module};
...
compile(ViewPath, Module, HelperDirModule, TranslatorPid, OutDir,
        CompilerOptions, Locales, DocRoot, TagHelpers, 
        FilterHelpers, ExtraTagHelpers, ExtraFilterHelpers) ->
    CompileParams = [
        {doc_root, DocRoot},
        {custom_tags_modules, TagHelpers ++ ExtraTagHelpers ++ 
            [boss_erlydtl_tags, HelperDirModule]},
